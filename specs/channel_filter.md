...
Set `DISCORD_FILTER_CHANNEL_IDS` in `.env` to a comma-separated list of Discord channel snowflake IDs:

```
DISCORD_FILTER_CHANNEL_IDS=123456789012345678,987654321098765432
```

If `DISCORD_FILTER_CHANNEL_IDS` is unset or empty, the bot processes messages from **all channels** in all guilds (existing behavior).

## Behavior

| `DISCORD_FILTER_CHANNEL_IDS` value | Behavior |
|---|---|
...
- Channel IDs are parsed once at startup and held in memory. Restarting the bot is required to pick up changes to `DISCORD_FILTER_CHANNEL_IDS`.
